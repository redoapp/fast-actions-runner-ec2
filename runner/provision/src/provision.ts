...
import { awsCredentialsProvider } from "./aws";
...
const githubClient = new Octokit({
  ...
  auth: { id: githubAppId, privateKey: githubPrivateKey },
});
...
async function createInstance({
      ...
      TagSpecifications: [
        {
          ...
          Tags: [
            { Key: "ActionsRunner:ProvisionerId", Value: provisionerId },
            {
              Key: "ActionsRunner:RunnerCreate:Region",
              Value: runnerCreateRegion,
            },
          ],
        },
      ],
  ...
  for (const instance of output.Instances!) {
    ...
    url.searchParams.set("provisioner_id", instance.InstanceId!);
    await ec2Client.send(
      new CreateTagsCommand({
        Resources: [instance.InstanceId!],
        Tags: [
          { Key: "ActionsRunner:RunnerCreate:Url", Value: url.toString() },
        ],
      }),
    );
  }
  ...
  await dynamodbClient.send(
    new BatchWriteItemCommand({
      RequestItems: {
        [instanceTableName]: output.Instances!.map((instance) => ({
          PutRequest: {
            Item: {
...
async function* getInstances({
  ...
  for await (const result of paginateDescribeInstances(
    ...
    {
      Filters: [
        { Name: "tag:ActionsRunner:Provisioner", Values: [provisionerId] },
        {
