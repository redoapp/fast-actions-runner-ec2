...
import { githubWebhook, provisionerInstallationClient } from "./github";
...
export async function refreshRunner({ instanceId }: { instanceId: string }) {
  const instanceOutput = await dynamodbClient.send(
    new GetItemCommand({
      ...
      ProjectionExpression: "ActiveAt, RunnerId, ProvisionerId",
    }),
  );
  const instanceItem = instanceOutput.Item;
  if (!instanceItem || !instanceItem.RunnerId) {
    return;
  }
  ...
  const activeAt = instanceItem.ActiveAt && instantRead(instanceItem.Active);
  const runnerId = numberRead(instanceItem.RunnerId);
